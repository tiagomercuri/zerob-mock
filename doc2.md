...
Adição
=========
O endpoint GET /api/pesquisa?idPesquisa=1&idSistema=1&etapa=1 não é chamado
O front não usa esse endpoint em nenhum lugar. Pode ignorar por enquanto.

Endpoints que o front realmente chama (em ordem de uso)
1. GET /api/pesquisa/lista?status=1 — ✅ OK (respostas que o colega postou estão corretas)
Retorna ResumoPesquisa[] com idPesquisa, nome, status, etc.

2. GET /api/pesquisa/lista/areas — ✅ OK (respostas que o colega postou estão corretas)
Retorna Area[] com idArea, nome, etc.

3. GET /api/pesquisa/cadastro/{id} — este é o crítico
O front chama buscarPorId(id) no componente de etapa para carregar as questões. O response deve ser um objeto único (não array):
...
Atenção crítica: o campo etapa dentro de cada questão tem que ser número (1, 2, 3 ou 4), não string como "Usabilidade". O front filtra assim:


// etapa.component.ts linha 88
pesquisa.questoes.filter((q) => q.etapa === this.idEtapaAtual())
// idEtapaAtual() é 1, 2, 3 ou 4
Se chegar "etapa": "Usabilidade", nenhuma questão aparece na tela.

4. GET /api/pesquisa/lista/etapas — metadados das etapas

[
  { "idEtapa": 1, "nome": "Usabilidade", "descricao": "...", "icone": "touch_app", "ordem": 1 },
  { "idEtapa": 2, "nome": "Eficiência",  "descricao": "...", "icone": "speed",     "ordem": 2 },
  { "idEtapa": 3, "nome": "Aprendizado", "descricao": "...", "icone": "school",    "ordem": 3 },
  { "idEtapa": 4, "nome": "Eficácia",    "descricao": "...", "icone": "verified",  "ordem": 4 }
]
5. GET /api/pesquisa/lista/sistemas — lista de sistemas para seleção
6. GET /api/pesquisa/resposta/{idPesquisa}/consulta?matricula=C123456 — verifica se já respondeu
7. POST /api/pesquisa/resposta — grava as respostas ao final
Resumo do que o colega precisa verificar: o endpoint GET /api/pesquisa/cadastro/{id} retorna um objeto (não array) com questoes[].etapa como número inteiro.